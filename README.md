# MiniBlog: Your Junk-Free, Personalized Information and Podcasts

MiniBlog manages your trusted articles and social media platform sources. It generates podcasts from the content you trust and curate. It handles the complete pipeline, from data collection and analysis to the production of scripts and visuals.
...
- [How to Use MiniBlog](#how-to-use-miniblog)
...
- [Cloud Options](#cloud-options)
  - [MiniBlog Cloud Features](#miniblog-cloud-features)
...
Before installing MiniBlog, ensure you have:
...
git clone https://github.com/arun477/miniblog.git
cd miniblog
...
# Navigate to the miniblog directory if not already there
cd miniblog  # Skip if already in the miniblog folder
...
Create a `.env` file in the `/miniblog` directory with your API keys:
...
MiniBlog offers flexibility in how you interact with the system:
...
MiniBlog includes several specialized processors for different content sources:
...
Extend MiniBlog's capabilities by adding your own content processors:
...
MiniBlog's AI system is built on the [agno](https://github.com/agno-agi/agno) framework and includes:
...
MiniBlog's search agent has full browser automation capabilities through the [browseruse](https://browser-use.com/) library, enabling web research and automated data collection from any website.
...
1. **Navigate to Social Tab** in the MiniBlog web interface
...
4. **Session Persistence** - MiniBlog will use these authenticated sessions for future automated searches
...
MiniBlog currently supports automated monitoring for:
...
1. **Navigate to the Voyager Tab** in the MiniBlog web interface
...
MiniBlog supports easy expansion to additional platforms:
...
MiniBlog supports multiple text to speech options:
...
MiniBlog can be integrated with other platforms.

### Slack Integration

MiniBlog's Slack integration enables you to interact with the AI agent directly from your Slack workspace. Each conversation with MiniBlog creates a dedicated Slack thread for the session.

**Key Feature:**

- Direct messaging with MiniBlogAI in Slack channels

### Setting Up Slack App

To integrate MiniBlog with your Slack workspace, you need to create a Slack app in Socket Mode:

#### Step 1: Create Slack App

1. Visit [Slack API Apps](https://api.slack.com/apps) and click "Create New App"
2. Choose "From scratch" and provide:
   - **App Name**: MiniBlogAI (or your preferred name)
...
Your Slack app requires specific permissions to function properly with MiniBlog:

#### OAuth & Permissions - Bot Token Scopes

Add the following scopes under "OAuth & Permissions" → "Bot Token Scopes":

- **`app_mentions:read`** - View messages that directly mention @MiniBlogAI in conversations that the app is in
- **`assistant:write`** - Allow MiniBlogAI to act as an App Agent
- **`channels:history`** - View messages and other content in public channels that MiniBlogAI has been added to
- **`channels:read`** - View basic information about public channels in a workspace
- **`chat:write`** - Send messages as @MiniBlogAI
- **`files:read`** - View files shared in channels and conversations that MiniBlogAI has been added to
- **`files:write`** - Upload, edit, and delete files as @MiniBlogAI
- **`im:read`** - View basic information about direct messages that MiniBlogAI has been added to
...
Add your Slack tokens to the `.env` file in the `/miniblog` directory:
...
2. Add MiniBlogAI to the channels where you want to use it
3. You can also send direct messages to MiniBlogAI

#### Step 2: Start Slack Integration

```bash
# Navigate to miniblog directory
cd miniblog
...
#### Step 3: Interact with MiniBlogAI

**In Slack Channels:**

- Mention @MiniBlogAI to start a conversation
- Each mention creates a new thread for context continuity
- Example: `@MiniBlogAI Can you help me analyze the latest news about AI developments?`
...
For accessing MiniBlog from outside your local network (workaround):
...
MiniBlog doesn't include an authentication layer yet. Authentication will be added in the next version.

## Cloud Options

### MiniBlog Cloud Features

Coming Soon!

✅ Cloud version of MiniBlog
...
Some of the data collection features rely on browser automation, which sometimes won't work properly in server environments. While MiniBlog will still function, some browser dependent features may not work in server environments without proper browser setup.

## Updates

🚀 **[Repo](https://github.com/arun477/miniblog)**
