...
def execute_simple_search(cursor, terms, limit):
    base_query = """
...
def get_article_categories(cursor, article_id):
    try:
        cursor.execute("SELECT category_name FROM article_categories WHERE article_id = ?", (article_id,))
