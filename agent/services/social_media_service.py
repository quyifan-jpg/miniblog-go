...
class SocialMediaService:
    ...
    async def _cached(self, cache_key: str, ttl_seconds: int, loader):
        return await async_redis_cache.get_or_set_json_singleflight(
            key=cache_key,
    ...
    async def get_posts(
        ...
        try:
            ...
            total_posts = await social_media_db.execute_query(
    ...
    async def get_sentiments(self, date_from: str | None = None, date_to: str | None = None) -> list[dict[str, Any]]:
    ...
    async def get_top_users(
        ...
        async def _loader():
            query_parts = [
                "SELECT user_handle, user_display_name, COUNT(*) as post_count",
                "FROM posts",
            ...
            if date_to:
                query_parts.append("AND post_timestamp <= ?")
                params.append(date_to)
            query_parts.extend(["GROUP BY user_handle, user_display_name", "ORDER BY post_count DESC", "LIMIT ?"])
            params.append(limit)
            query = " ".join(query_parts)
            return await social_media_db.execute_query(query, tuple(params), fetch=True)
    ...
    async def get_categories(self, date_from: str | None = None, date_to: str | None = None) -> list[dict[str, Any]]:
        ...
        try:
            ...
            async def _loader():
                ...
                if date_to:
                    query_parts.append("AND post_timestamp <= ?")
                    params.append(date_to)
                rows = await social_media_db.execute_query(" ".join(query_parts), tuple(params), fetch=True)
                ...
                for row in rows:
                    ...
                    for category in categories:
                        ...
                        entry["total_count"] += 1
                        if sentiment in ("positive", "negative", "neutral", "critical"):
                            entry[f"{sentiment}_count"] += 1

                result = sorted(category_counts.values(), key=lambda x: x["total_count"], reverse=True)
    ...
    async def get_trending_topics(
        ...
        try:
            ...
            async def _loader():
                ...
                result = sorted(topic_counts.values(), key=lambda x: x["total_count"], reverse=True)[:limit]
                for topic in result:
    ...
    async def get_sentiment_over_time(
        self, date_from: str | None = None, date_to: str | None = None, platform: str | None = None
    ) -> list[dict[str, Any]]:
    ...
    async def get_influential_posts(
        self, sentiment: str | None = None, limit: int = 5, date_from: str | None = None, date_to: str | None = None
    ) -> list[dict[str, Any]]:
    ...
    async def get_engagement_stats(self, date_from: str | None = None, date_to: str | None = None) -> dict[str, Any]:
        ...
        try:
            ...
            async def _loader():
                ...
                avg_replies = result_dict.get("avg_replies") or 0
                avg_retweets = result_dict.get("avg_retweets") or 0
                avg_likes = result_dict.get("avg_likes") or 0
                avg_bookmarks = result_dict.get("avg_bookmarks") or 0
                result_dict["avg_replies"] = avg_replies
                result_dict["avg_retweets"] = avg_retweets
                result_dict["avg_likes"] = avg_likes
                result_dict["avg_bookmarks"] = avg_bookmarks
                result_dict["avg_views"] = result_dict.get("avg_views") or 0
                result_dict["max_replies"] = result_dict.get("max_replies") or 0
                result_dict["max_retweets"] = result_dict.get("max_retweets") or 0
                result_dict["max_likes"] = result_dict.get("max_likes") or 0
                result_dict["max_bookmarks"] = result_dict.get("max_bookmarks") or 0
                ...
                platforms = await social_media_db.execute_query(
                    " ".join(platform_query_parts), tuple(params), fetch=True
                )
            ...
            return (
                ...
                else {"avg_engagement": 0, "total_posts": 0, "unique_authors": 0}
            )
        except Exception as e:
            if isinstance(e, HTTPException):
                raise e
