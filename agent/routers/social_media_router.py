...
async def read_posts(
    ...
    platform: str | None = Query(None, description="Filter by platform (e.g., x.com, instagram)"),
    user_handle: str | None = Query(None, description="Filter by user handle"),
    ...
    search: str | None = Query(None, description="Search in post text, user display name, or handle"),
):
    """
...
def _run_browser_setup_background(sites: list[str] | None = None):
...
async def setup_browser_session(
    sites: list[str] | None = Query(None, description="List of sites to setup sessions for"),
):
    """
